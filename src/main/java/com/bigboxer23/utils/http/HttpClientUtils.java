...
public class HttpClientUtils {
	...
	public static String execute(HttpRequestBase theRequestBase) {
		...
		try (CloseableHttpResponse aResponse = HttpClientUtils.getInstance().execute(theRequestBase)) {
			String aResponseString =
					new String(ByteStreams.toByteArray(aResponse.getEntity().getContent()), Charsets.UTF_8);
	...
	/**
	 * Return a cached http client which has good default timeouts, and ignores self signed SSL
	 * certs for internal HTTPS
	 *
	 * @return
	 */
